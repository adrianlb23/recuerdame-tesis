...
export default function AdminEditPerfume({
  ...
  onClose,
  catalog = "hombre",
  perfume,              // objeto existente {numero, nombre, ...}
  onUpdated,
}) {
  const isNicho = catalog === "nicho";
  const numeroOriginal = perfume?.numero;
  ...
  useEffect(() => {
    if (open && perfume) {
      setForm({
        numero: perfume.numero ?? "",
        nombre: perfume.nombre ?? "",
        marca: perfume.marca ?? "",
        tipo: perfume.tipo ?? "",
        tipo2: perfume.tipo2 ?? "",
        url: perfume.url ?? "",
        dispo: perfume.dispo ?? true,
        genero: perfume.genero ?? "unisex",
        ocasion: Array.isArray(perfume.ocasion) ? perfume.ocasion : (perfume.ocasion ? [perfume.ocasion] : []),
        clima: perfume.clima ?? "versatil",
        edad: perfume.edad ?? "ambos",
      });
      setErr("");
      setLoading(false);
    }
  ...
  const canSubmit = useMemo(() => {
    if (!form.nombre || !form.marca) return false;
    if (isNicho) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const payload = {
        // número: lo dejamos opcional; si quieres permitir cambiarlo, habilita input y envíalo
      ...
      const { data } = await api.put(
        `/api/admin/catalogo/${catalog}/${numeroOriginal}`,
        payload
      );
      onUpdated?.(data?.resultado || null);
      onClose?.();
    } catch (e) {
      console.error(e);
      setErr("No se pudo actualizar el perfume. Revisa autenticación y campos.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="modal-backdrop" onClick={onClose} aria-hidden>
      <div className="form-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
        <div className="form-header">
          <h3>Editar perfume #{numeroOriginal} ({catalog})</h3>
          ...
          <div className="form-grid">
            <label className="form-field">
              ...
              <input
                ...
                value={form.numero}
                onChange={onChange}
                disabled     // habilita si deseas permitir cambiar el número
              />
            </label>
